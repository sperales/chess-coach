...
try{
  ...
  foreach($pgns as $one){ $uid=hash('sha256',$one); $dt=pgn_date(pgn_tag($one,'Date')); $data=[pgn_tag($one,'White'),pgn_tag($one,'Black'),pgn_tag($one,'Result'),result_for_user($one,$username),$dt,pgn_tag($one,'Event'),pgn_tag($one,'Site'),$one,$u['id'],$uid,'chess.com'];
    try{
