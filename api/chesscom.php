...
try{
  ...
  foreach($pgns as $one){ $uid=hash('sha256',$one); $dt=pgn_date(pgn_tag($one,'Date')); $data=[pgn_tag($one,'White'),pgn_tag($one,'Black'),pgn_tag($one,'Result'),result_for_user($one,$username),$dt,pgn_tag($one,'Event'),pgn_tag($one,'Site'),pgn_eco_code($one),pgn_opening_name($one),$one,$u['id'],$uid,'chess.com'];
    try{
      $st=db()->prepare('INSERT INTO games (white_player,black_player,result_raw,user_result,played_at,event_name,site,eco_code,opening_name,pgn,user_id,game_uid,source) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)');
      $st->execute($data);
