...
if($action==='import'){
  ...
  foreach($pgns as $pgn){
    ...
    $data=[pgn_tag($pgn,'White'),pgn_tag($pgn,'Black'),pgn_tag($pgn,'Result'),result_for_user($pgn,$u['username']),$dt,pgn_tag($pgn,'Event'),pgn_tag($pgn,'Site'),pgn_eco_code($pgn),pgn_opening_name($pgn),$pgn,$u['id'],$uid,'manual'];
    try{
